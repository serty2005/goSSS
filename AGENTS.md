...
7. Ожидание загрузки показывается только единым индикатором в header (`GlobalLoadingIndicator`, слева от кнопки темы). Он сам учитывает запросы и мутации TanStack Query; ожидания вне Query регистрировать через `useReportGlobalLoading` или `LoadingPlaceholder report`. Не добавлять на страницы `Spin`, overlay `loading` у таблиц/списков и текстовые «Загрузка...»; `loading` допустим только у кнопок действий и выпадающих списков. Фоновый опрос, который не должен мигать индикатором, помечать `meta: { globalLoading: false }`.
8. Не строить данные страницы веером однотипных запросов по списку сущностей: production Traefik ограничивает запросы по IP (`etalon-ratelimit`). Нужна агрегированная ручка backend (пример — `GET /api/companies/{id}/network`).
