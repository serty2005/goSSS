...
- `tickets` - заявки, история, комментарии, вложения, файловые связи, локальные чеклисты и шаблоны чеклистов, отложенные события.
...
- `/admin/users`, `/admin/translations`, `/admin/telephony`, `/admin/synchronizations`, `/admin/checklist-templates` - администрирование;
...
- `/profile`, `/translations`, `/users`, `/checklist-templates`;
...
### 8.4. Файлы, материалы и чеклисты тикета

Загрузка файлов тикета идет через `POST /api/tickets/{id}/attachments` (multipart `files`) с необязательным полем `relation`:

- без поля или `direct` - прямое вложение, отображается во вкладке «Вложения»;
- `inline_comment` - файл или картинка, вставленные в комментарий; при сохранении комментария файл привязывается к нему через `ticket_file_links.comment_uuid`;
- `inline_description` - файл или картинка в описании тикета.

Inline-файлы не показываются во вложениях. Если файл убран из текста комментария или описания и сохранен, либо удален весь комментарий, файл удаляется из `TICKET_STORAGE_PATH` и из `file_assets`, при условии что он не используется в других текстах тикета и не является прямым вложением. Копии, уже отправленные в Naumen, Bitrix24 или Pyrus, не удаляются. Старые файлы, загруженные до разделения типов связей, остаются прямыми вложениями.

`GET /api/materials/company-scope/{companyID}` собирает материалы компании, ее родительской компании и оборудования компании (серверы, РС, ФР) с указанием источника (`scope`: `company`, `parent`, `equipment`). Вкладка «Материалы» в тикете показывается, только если такие материалы есть; ссылка «Открыть в редакторе» ведет на карточку источника с параметрами `?tab=materials&material=<id>&edit=1`.

Чеклист тикета локальный и не синхронизируется с внешними системами, закрытие тикета не блокирует:

- `GET /api/tickets/{id}/checklist` - пункты плоским списком (`parent_id`, `position`, исполнители, отметка выполнения);
- `POST /api/tickets/{id}/checklist/items` - добавление; каждая непустая строка `title` становится отдельным пунктом;
- `PATCH /api/tickets/{id}/checklist/items/{itemID}` - текст, `is_done`, `assignee_ids`;
- `DELETE /api/tickets/{id}/checklist/items/{itemID}` - удаление пункта вместе с подпунктами;
- `POST /api/tickets/{id}/checklist/reorder` - порядок и перенос между уровнями (`parent_id`, `ordered_ids`);
- `POST /api/tickets/{id}/checklist/apply-template` - добавление пунктов шаблона в конец чеклиста.
