...
- Важный открытый долг: bootstrap-регистрация пока сразу выдаёт токены; обязательное ручное подтверждение пользователя или администратора ещё не реализовано как финальный production-flow.
...
- Временный риск текущей реализации: если heartbeat приходит по `agent_uuid`, которого нет в `agents`, `agent_service` сейчас может автосоздать запись агента и выставить `status=active`. Это диагностический обходной сценарий, который нужно убрать после внедрения approval-flow регистрации.
