...
type BitrixWebhookHandler struct {
	service services.BitrixIncomingService
}

func NewBitrixWebhookHandler(service services.BitrixIncomingService) *BitrixWebhookHandler {
	return &BitrixWebhookHandler{service: service}
}

func (h *BitrixWebhookHandler) HandleWebhook(w http.ResponseWriter, r *http.Request) {
	if h == nil || h.service == nil {
		response.RespondWithError(w, http.StatusServiceUnavailable, "входящая синхронизация Bitrix24 недоступна")
		return
	}
	...
	if err = h.service.HandleWebhook(r.Context(), rawBody, form); err != nil {
		switch {
		case errors.Is(err, services.ErrBitrixWebhookUnauthorized):
			response.RespondWithError(w, http.StatusUnauthorized, err.Error())
		case errors.Is(err, services.ErrBitrixWebhookBadRequest):
