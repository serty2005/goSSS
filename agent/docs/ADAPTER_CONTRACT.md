...
Этот документ описывает только adapter-based task contract.
Для нового task type `saga_run`, который исполняется самим `core-agent`, действует отдельный документ:

- [Saga Runtime](./saga_runtime.md)
...
- не должен выполнять self-update `core-agent` путём прямой замены binary;
- если оркестрации нужен self-update агента, это должен делать native step `core-agent`, а не внешний адаптер.
