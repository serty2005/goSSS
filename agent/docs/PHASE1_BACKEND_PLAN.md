...
- Локальный bootstrap-конфиг теперь хранится во внешнем JSON-файле `agent-config.json`.
- По умолчанию агент ищет `agent-config.json` рядом со своим exe-файлом.
- Путь к конфигу можно переопределить через `--config <path>` или переменную окружения `ETALON_AGENT_CONFIG`.
- Из внешнего конфига читаются минимум:
  - `brand_name`
  - `server_url`
  - `bootstrap_api_key`
...
6. Механизм ручного подтверждения регистрации/активации агента пока не доведён до production-состояния:
   - bootstrap-регистрация сразу выдаёт токены;
   - heartbeat-контур всё ещё допускает автосоздание записи агента;
   - перед production-переносом collectors нужен явный approval-flow пользователя или администратора.
