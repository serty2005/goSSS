...
func New(version string, bridge protocol.Bridge) *App {
	return &App{
...
func (a *App) handleHealth(ctx context.Context) int {
	...
	if err != nil {
		response := contract.HealthResponse{
			Status:  "error",
			Message: fmt.Sprintf("Не удалось проверить готовность окружения Mitsu: %v", err),
	...
	case !probe.DriverPresent:
		...
		response.Message = "MitsuCube.exe не найден"
	case probe.DriverVersion == "":
		response.Status = "degraded"
		response.Message = "MitsuCube.exe найден, но его версия не определена"
	}
