...
func NewService(bridge protocol.Bridge) *Service {
...
func (s *Service) Collect(ctx context.Context, endpoints []domain.Endpoint) []domain.DeviceResult {
	...
	for _, endpoint := range endpoints {
		...
		if err != nil {
			...
			result.Message = "Не удалось собрать данные по ККТ Mitsu"
		...
		result.Message = "Данные по ККТ Mitsu собраны"
		if len(warnings) > 0 {
			result.Message = "Данные по ККТ Mitsu собраны с предупреждениями"
		}
