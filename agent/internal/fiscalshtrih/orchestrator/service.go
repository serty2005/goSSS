...
func NewService(bridge drvfr.Bridge) *Service {
...
func (s *Service) Collect(ctx context.Context, endpoints []domain.Endpoint) []domain.DeviceResult {
	...
	for _, endpoint := range endpoints {
		...
		if err != nil {
			...
			result.Message = "Не удалось собрать данные по ККТ Штрих"
		...
		result.Message = "Данные по ККТ Штрих собраны"
		if len(warnings) > 0 {
			result.Message = "Данные по ККТ Штрих собраны с предупреждениями"
		}
