...
func readDLLVersion(path string) (string, error) {
	pathPtr, err := windows.UTF16PtrFromString(path)
	if err != nil {
		return "", fmt.Errorf("не удалось подготовить путь к DLL: %w", err)
	}
	...
	if size == 0 {
		...
		return "", fmt.Errorf("version info для DLL отсутствует")
	}
