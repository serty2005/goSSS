...
func New(version string, bridge libfptr.Bridge) *App {
	return &App{
...
func (a *App) handleHealth(ctx context.Context) int {
	...
	if err != nil {
		response := contract.HealthResponse{
			Status:  "error",
			Message: fmt.Sprintf("Не удалось проверить готовность драйвера Атол: %v", err),
	...
	response := contract.HealthResponse{
		...
		Details: map[string]any{
			...
			"search_paths":   probe.SearchPaths,
		},
	}
	if probe.DriverPath != "" {
		response.Details["driver_path"] = probe.DriverPath
	}
	if probe.DriverVersion != "" {
		response.Details["driver_version"] = probe.DriverVersion
	}
	if probe.DriverVariant != "" {
		response.Details["driver_variant"] = string(probe.DriverVariant)
	}
	if probe.Message != "" {
	...
	case !probe.DriverPresent:
		...
		response.Message = "Драйвер Атол не найден"
	case probe.DriverVersion == "":
		response.Status = "degraded"
		response.Message = "Драйвер Атол найден, но его версия не определена"
	}
